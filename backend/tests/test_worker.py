...
def test_recover_interrupted_jobs_requeues_running(temp_env):
    ...
    with Session(get_engine()) as session:
        v = Video(youtube_id="AAAAAAAAAAA", title="t",
                  status=VideoStatus.downloading)
        session.add(v)
        session.commit()
        session.refresh(v)
        # A job the previous process was mid-download on when it died.
        session.add(DownloadJob(video_id=v.id, state=JobState.running,
                                progress_pct=42.0))
...
def test_combined_total_sums_streams():
    ...
    assert _combined_total({"requested_formats": [
        {"filesize_approx": 1000}, {}]}) == 1000
...
def test_dest_dir_layout():
    ...
    v = Video(youtube_id="AAAAAAAAAAA", title="t", source_type=SourceType.match,
              team_keys="frc254", year=2026, event_key="2026necmp")
    assert _dest_dir(root, v) == Path("/library/frc254/2026/2026necmp")

    # Shared match video uses the first (discovering) team key.
    v = Video(youtube_id="BBBBBBBBBBB", title="t", source_type=SourceType.match,
              team_keys="frc254,frc118", year=2026, event_key="2026necmp")
    assert _dest_dir(root, v) == Path("/library/frc254/2026/2026necmp")

    # Season/district (event_vod) video keeps the flat year/event layout.
    v = Video(youtube_id="CCCCCCCCCCC", title="t",
              source_type=SourceType.event_vod, year=2026, event_key="2026necmp")
    assert _dest_dir(root, v) == Path("/library/2026/2026necmp")

    # A match video with no team keys falls back to the flat layout.
    v = Video(youtube_id="DDDDDDDDDDD", title="t", source_type=SourceType.match,
              team_keys="", year=2026, event_key="2026necmp")
    assert _dest_dir(root, v) == Path("/library/2026/2026necmp")
