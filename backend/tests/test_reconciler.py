...
def _make_video(session, youtube_id, path, ext, vcodec, size):
    ...
    v = Video(
        ...
        status=VideoStatus.completed, present=True,
        file_path=str(path), orig_ext=ext, orig_vcodec=vcodec, orig_size=size,
        current_ext=ext, current_vcodec=vcodec, current_size=size,
...
def test_transcode_detected_on_extension_change(temp_env, monkeypatch):
    ...
    monkeypatch.setattr(
        reconciler, "ffprobe",
        lambda p: MediaInfo(container="matroska", vcodec="av1", size=60,
                            duration=120),
    )
...
def test_missing_file_flagged_not_redownloaded(temp_env):
    ...
    with Session(get_engine()) as session:
        ...
        v = session.exec(
            select(Video).where(Video.youtube_id == "BBBBBBBBBBB")
        ).first()
